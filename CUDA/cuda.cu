...
int main(int argc, char **argv) {
    int n = 0, k = 0, all_graphs = 0, batch_size = DEFAULT_BATCH_SIZE, block_size_override = 0, verbose = 0;
    int i = 1;

    if (argc < 3) {
        printf("%s: n k [-a] [-b batch] [-B blockSize] [-d]\n", argv[0]);
        return 1;
    }
    ...
    if (verbose) printf("GPU: %s, max threads per block: %d, using: %d\n",
    ...
    if (!h_wyniki) {
        ...
        free(h_bufory); fclose(rf); pclose(fp); return 1;
    }
...

    {
        ...
        while (processed_count < skip_count) {
            if (fgets(h_bufory, BUFSIZE, fp) == NULL) break;
            processed_count++;
        }
    ...
    do {
        for (graphs = 0; graphs < batch_size; graphs++) {
            if (stop_flag) break;
            if (fgets(&h_bufory[graphs * BUFSIZE], BUFSIZE, fp) == NULL)
                break;
        }
